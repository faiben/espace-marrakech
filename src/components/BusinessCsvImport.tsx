...
export function BusinessCsvImport({ onClose }: BusinessCsvImportProps) {
  ...
  const mapRow = (values: string[], headerMap: Record<string, number>): CsvRow => {
    ...
    if (!address) errors.push(t.adresseManquante);
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-emerald-100">
                      ...
                      <th className="text-left py-2 px-2 text-navy-500 font-medium">{t.categorieCol}</th>
                      <th className="text-left py-2 px-2 text-navy-500 font-medium">{t.telephoneCol}</th>
                      <th className="text-left py-2 px-2 text-navy-500 font-medium">{t.packageCol}</th>
