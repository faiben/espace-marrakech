...
export function BusinessCsvImport({ onClose }: BusinessCsvImportProps) {
  ...
  const mapRow = (values: string[], headerMap: Record<string, number>): CsvRow => {
    ...
    if (!address) errors.push(L("Adresse manquante","العنوان مفقود"));
    if (!phone) errors.push(L("Téléphone manquant","الهاتف مفقود"));

    return { nameFr, nameAr, descriptionFr: descriptionFr || nameFr, descriptionAr: descriptionAr || nameAr, category, address, phone, email, lat, lng, website, whatsapp, packageType, raw, errors };
  };
            ...
            <div className="text-center py-10">
              ...
              <div className="mt-8 text-left max-w-lg mx-auto">
                ...
                <div className="grid grid-cols-2 gap-1 text-xs text-navy-500">
                  {["name_fr / name / nom", "name_ar / nom_ar", "description / description_fr", "description_ar", "category / type / secteur", "address / adresse", "phone / telephone / gsm", "email / mail", "lat / latitude", "lng / longitude", "website / site", "whatsapp", "package / plan / tier"].map((col) => (
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-emerald-100">
                      ...
                      <th className="text-left py-2 px-2 text-navy-500 font-medium">{ L("Catégorie","الفئة") }</th>
                      <th className="text-left py-2 px-2 text-navy-500 font-medium">{ L("Téléphone","الهاتف") }</th>
                      <th className="text-left py-2 px-2 text-navy-500 font-medium">{ L("Package","الباقة") }</th>
          ...
          {step === "done" && (
            <div className="text-center py-10">
              ...
              <p className="text-navy-500">
                {imported} { L("commerces importés","تجارة مستوردة") }
              </p>
            </div>
          )}
        </div>
        ...
        <div className="sticky bottom-0 bg-white border-t border-emerald-100 px-6 py-4 flex justify-end gap-3">
          ...
          {step === "preview" && validRows.length > 0 && (
            <button onClick={handleImport} className="px-5 py-2 rounded-lg text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 transition-colors">
              { L(`Importer ${validRows.length} lignes`,`استيراد ${validRows.length} صنف`) }
            </button>
          )}
          {step === "done" && (
