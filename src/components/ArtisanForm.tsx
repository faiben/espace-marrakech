      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-emerald-100 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
          <h3 className="font-bold text-navy-800 text-lg">
            {artisan ? (L("Modifier l'artisan","تعديل الحرفي")) : (L("Ajouter un artisan","إضافة حرفي"))}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className={labelClass}>{ L("Spécialité","التخصص") }</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>{ L("Adresse (FR)","العنوان (FR)") }</label>
              <input required className={inputClass} value={form.addressFr} onChange={(e) => setForm({ ...form, addressFr: e.target.value })} />
            </div>
            <div>
              <label className={labelClass}>{ L("Adresse (AR)","العنوان (عربي)") }</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>{ L("Missions réalisées","المهام المنجزة") }</label>
            ...
            <div className="flex items-end pb-2">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm font-medium text-navy-700">{ L("Visible","مرئي") }</span>
              </label>
            </div>
          </div>
