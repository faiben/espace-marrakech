      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-emerald-100 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
          <h3 className="font-bold text-navy-800 text-lg">
            {business ? (L("Modifier le commerce","تعديل Commerce")) : (L("Ajouter un commerce","إضافة commerce"))}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>{ L("Catégorie","الفئة") }</label>
            ...
            <div>
              <label className={labelClass}>{ L("Package","الباقة") }</label>
          ...
          <div>
            <label className={labelClass}>{ L("Adresse","العنوان") }</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelClass}>{ L("Email","البريد") }</label>
              <input required type="email" className={inputClass} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>{ L("Site web","الموقع") }</label>
          ...
          <div>
            <label className={labelClass}>{ L("Images (une URL par ligne)","الصور (رابط لكل سطر)") }</label>
          ...
          <div>
            <label className={labelClass}>{ L("Vidéo (URL YouTube embed)","الفيديو") }</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className={labelClass}>{ L("Note","التقييم") }</label>
          ...
          <div>
            <label className={labelClass}>{ L("ID du propriétaire (optionnel)","معرف المالك (User ID)") }</label>
