...
export function ArtisanRequestForm({ artisan, onSave, onClose }: ArtisanRequestFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      artisanName: lang === "ar" ? artisan.nameAr : artisan.nameFr,
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 text-center">
          ...
          <p className="text-navy-500 mb-6">
            { L("Votre demande a été envoyée. Notre équipe vous contactera bientôt.","تم إرسال طلبك. سيتصل بك فريقنا قريباً.") }
          </p>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-emerald-100 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
          <h3 className="font-bold text-navy-800 text-lg">
            { L("Demander un artisan","طلب حرفي") }
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-3 p-3 bg-primary-50 rounded-xl">
            ...
            <div>
              <p className="font-medium text-navy-800 text-sm">{ lang === "ar" ? artisan.nameAr : artisan.nameFr }</p>
          ...
          <div>
            ...
            <textarea required rows={3} className={inputClass} value={form.descriptionFr} onChange={(e) => setForm({ ...form, descriptionFr: e.target.value })}
              placeholder={ L("Décrivez votre problème...","صفي ما تحتاج...") } />
          </div>
          <div>
            <label className={labelClass}>{ L("Décrivez votre besoin (AR)","وصف المشكلة (بالعربية)") }</label>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-700">
            { L("Vos coordonnées seront partagées avec l'artisan uniquement après acceptation de la mission.","سيتم مشاركة معلومات الاتصال الخاصة بك مع الحرفي فقط بعد الموافقة على المهمة.") }
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 transition-colors">
              <Send size={14} /> { L("Envoyer la demande","إرسال الطلب") }
            </button>
          </div>
        </form>
      </div>
