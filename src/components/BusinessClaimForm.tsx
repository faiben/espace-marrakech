...
export function BusinessClaimForm({ business, onClose }: BusinessClaimFormProps) {
  ...
  const handleClaim = () => {
    ...
    addClaim({
      ...
      businessName: lang === "ar" ? business.nameAr : business.nameFr,
      userId: user.id,
      userName: user.name,
  ...
  const handleVerify = () => {
    ...
    const message = encodeURIComponent(
      `Bonjour Espace Marrakech ! Je confirme la réclamation du commerce "${ lang === "ar" ? business.nameAr : business.nameFr }" (ID: ${business.id}). Mon numéro WhatsApp: ${whatsapp}. Package choisi: ${selectedPackage}.`
    );
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
          ...
          <div className="text-center">
            ...
            <h3 className="text-lg font-bold text-navy-800 mb-2">
              {existingClaim.status === "pending" && (L("Demande en cours","طلب قيد المراجعة"))}
              {existingClaim.status === "verified" && (L("Vérifié","تم التحقق"))}
              {existingClaim.status === "approved" && (L("Approuvé","تم القبول"))}
            </h3>
            <p className="text-sm text-navy-500">
              {existingClaim.status === "pending" && (L("Votre demande est en cours de traitement par l'administration.","سيتم مراجعة طلبك من قبل الإدارة"))}
              {existingClaim.status === "verified" && (L("Votre numéro WhatsApp a été vérifié.","تم التحقق من رقم الواتساب الخاص بك"))}
              {existingClaim.status === "approved" && (L("Vous êtes maintenant propriétaire de ce commerce.","تم قبولك كمالك لهذا commerce"))}
            </p>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-emerald-100 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
          <h3 className="font-bold text-navy-800 text-lg">
            { L("Réclamer ce commerce","مطالبة بالتجارة") }
          </h3>
        ...
        <div className="p-6">
          {step === "form" && (
            <div className="space-y-5">
              <div className="bg-primary-50 rounded-xl p-4">
                <p className="font-medium text-navy-800">{ lang === "ar" ? business.nameAr : business.nameFr }</p>
                <p className="text-xs text-navy-500 mt-1">{business.address}</p>
              </div>

              <div>
                <label className="block text-sm font-medium text-navy-700 mb-1">
                  { L("Numéro WhatsApp (requis)","رقم الواتساب (مطلوب)") }
                </label>
                <input
                  type="tel"
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-navy-700 mb-2">
                  { L("Choisir le package","اختر الباقة") }
                </label>
              ...
              >
                <MessageCircle size={18} /> { L("Envoyer via WhatsApp","إرسال عبر الواتساب") }
              </button>
            </div>
          )}
          ...
          {step === "verify" && (
            <div className="text-center space-y-4">
              ...
              <h4 className="font-bold text-navy-800">
                { L("Confirmation via WhatsApp","تأكيد عبر الواتساب") }
              </h4>
              <p className="text-sm text-navy-500">
                {isArabic
              ...
              <p className="text-xs text-navy-400">
                { L("Vous devez avoir WhatsApp installé sur ce numéro","يجب أن يكون لديك واتساب مثبت على هذا الرقم") }
              </p>
              <div className="flex gap-3 justify-center">
                ...
                >
                  <MessageCircle size={16} /> { L("Ouvrir WhatsApp","فتح الواتساب") }
                </button>
              </div>
            </div>
          )}
          ...
          {step === "done" && (
            <div className="text-center space-y-4">
              ...
              <h4 className="font-bold text-navy-800">
                { L("Envoyé avec succès!","تم الإرسال بنجاح!") }
              </h4>
              <p className="text-sm text-navy-500">
                { L("Votre demande sera examinée par l'administration. Vous pouvez suivre l'état de votre demande depuis votre compte.","سيتم مراجعة طلبك من قبل الإدارة. يمكنك متابعة حالة الطلب من حسابك.") }
              </p>
              <button
                onClick={onClose}
