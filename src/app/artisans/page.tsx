...
function ArtisansContent() {
  ...
  const handleNearMe = () => {
    ...
    if (!("geolocation" in navigator)) {
      alert(L("La géolocalisation n'est pas disponible","المتصفح لا يدعم تحديد الموقع"));
      return;
    }
    setLocating(true);
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        alert(L("Impossible d'obtenir la localisation. Vérifiez les paramètres de localisation.","تعذر تحديد الموقع. تأكد من تفعيل خدمات الموقع."));
        setLocating(false);
      },
  ...
  const filtered = useMemo(() => {
    let results = searchArtisans(query, selectedSpecialty || undefined, allArtisans).filter((a) => a.isVisible);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-navy-800 mb-2">{t.artisansHeadline}</h1>
        <p className="text-navy-600 max-w-2xl">{t.artisansSubheadline}</p>
        <p className="text-navy-500 text-sm mt-2">{filtered.length} {t.results}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-8">
        ...
        <a
          href={`https://wa.me/${settings.whatsappNumber.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(
            L("Bonjour, je souhaite des informations sur les services d'artisans","مرحباً، أريد الاستفسار عن خدمات الحرفيين")
          )}`}
          target="_blank"
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            {requestSent ? (
              <div className="text-center py-8">
                ...
                <p className="text-lg font-semibold text-navy-800">{ L("Votre demande a été envoyée !","تم إرسال طلبك!") }</p>
                <p className="text-sm text-navy-500 mt-1">{ L("Notre équipe vous contactera bientôt","سيتواصل معك فريقنا قريباً") }</p>
              </div>
            ) : (
              <form onSubmit={handleRequest} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-navy-700 mb-1">{ L("Spécialité","التخصص") }</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-navy-700 mb-1">
                    { L("Décrivez votre besoin","صف احتياجك") }
                  </label>
                  <textarea
                    required rows={4} value={requestForm.description}
                    onChange={(e) => setRequestForm({ ...requestForm, description: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-emerald-200"
                    placeholder={ L("Ex: Besoin d'un plombier pour réparer un robinet de cuisine","مثال: يحتاج سباك لإصلاح صنبور المطبخ") }
                  />
                </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        >
          ...
          {locating ? (L("Localisation...","جاري التحديد...")) : t.nearMe}
        </button>
