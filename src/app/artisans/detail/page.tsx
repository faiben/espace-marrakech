  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="bg-white rounded-2xl border border-emerald-100 card-shadow p-6 md:p-8">
        <div className="flex items-start gap-5 mb-6">
          ...
          <div className="flex-1">
            <h1 className="text-2xl md:text-3xl font-bold text-navy-800">
              { lang === "ar" ? artisan.nameAr : artisan.nameFr }
            </h1>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="text-center p-3 bg-navy-50 rounded-xl">
            ...
            <p className="text-xs text-navy-500">{ L("Missions","مهام منجزة") }</p>
          </div>
          <div className="text-center p-3 bg-navy-50 rounded-xl">
            <div className="flex items-center justify-center gap-1 mb-1">
              <Shield size={16} className="text-blue-500" />
            </div>
            <p className="text-xs text-navy-500">{ L("Vérifié","محقق") }</p>
          </div>
        </div>

        <p className="text-navy-600 mb-6">
          { lang === "ar" ? artisan.descriptionAr : artisan.descriptionFr }
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="flex items-center gap-3 p-3 bg-navy-50 rounded-xl">
            ...
            <div>
              <p className="text-xs text-navy-400">{ L("Adresse","العنوان") }</p>
              <p className="text-sm font-medium text-navy-700">{ lang === "ar" ? artisan.addressAr : artisan.addressFr }</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-blue-800">
              { L("Confidentialité protégée","الخصوصية محمية") }
            </p>
            <p className="text-xs text-blue-600 mt-1">
              { L("Notre équipe contactera l'artisan pour vous. Si indisponible, nous vous proposerons un autre artisan.","سيتطلع فريقنا الحرفي ومتاح. نحن نتصل بك عندما يكون متاحاً.") }
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            <Send size={18} /> { L("Demander cet artisan","طلب هذا الحرفي") }
          </button>
          <a
      ...
      <div className="bg-white rounded-2xl border border-emerald-100 card-shadow p-6 md:p-8 mt-6">
        <h3 className="font-bold text-navy-800 mb-4">{ L("Laisser un avis","تقييم") }</h3>
        {userHasRated && !reviewSubmitted ? (
          <p className="text-sm text-navy-500">{ L("Vous avez déjà évalué cet artisan.","لقد قمت بتقييم هذا الحرفي بالفعل") }</p>
        ) : reviewSubmitted ? (
          <p className="text-sm text-green-600 font-medium">{ L("Merci pour votre avis !","شكراً لتقييمك!") }</p>
        ) : (
          <form onSubmit={(e) => {
            e.preventDefault();
            ...
            <input
              ...
              placeholder={ L("Votre nom (optionnel)","اسمك (اختياري)") }
