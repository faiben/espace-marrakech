          ...
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold mb-6 leading-tight drop-shadow-lg">
              {isArabic ? (
                ...
                <>{t.heroTitle.replace("finally connected.", "").trim()} <span className="text-accent-400">finally connected.</span></>
              ) : (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/annuaire" className="text-primary-600 hover:text-primary-700 font-medium flex items-center gap-1">
              { L("Tout voir","الكل") } <ArrowRight size={16} className={isArabic ? "rotate-180" : ""} />
            </Link>
          </div>
          ...
          <div className="rounded-2xl overflow-hidden border border-emerald-200 card-shadow bg-navy-50 h-80 flex items-center justify-center relative">
            <div className="text-center">
              ...
              <Link href="/annuaire?map=true" className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary-600 text-white font-medium hover:bg-primary-700 transition-colors text-sm shadow">
                <MapPin size={16} /> { L("Ouvrir la carte","فتح الخريطة") }
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 bg-gradient-to-br from-primary-700 to-primary-600">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-3xl font-bold text-white mb-3">
            { L("Vous êtes commerçant ou artisan à Marrakech ?","هل أنت تاجر أو صنّاعي في مراكش؟") }
          </h2>
          <p className="text-emerald-100 text-lg mb-8 max-w-xl mx-auto">
            { L("Rejoignez des milliers de commerçants et trouvez des centaines de clients chaque jour. La formule gratuite est disponible !","انضم لآلاف التجار واستفد من وصول مئات الزوار يومياً. الخطّة المجانية متاحة!") }
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              { L("Voir l'annuaire","تصفح الدulaire") }
            </Link>
          </div>
        </div>
      </section>
