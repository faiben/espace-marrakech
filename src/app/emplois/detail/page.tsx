      ...
      <div className="bg-white rounded-2xl border border-emerald-100 card-shadow p-6 md:p-8">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-navy-800">
              { lang === "ar" ? job.titleAr : job.titleFr }
            </h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {job.salary && (
            <div className="flex items-center gap-3 p-3 bg-navy-50 rounded-xl">
              ...
              <div>
                <p className="text-xs text-navy-400">{ L("Salaire","الراتب") }</p>
          ...
          <div className="flex items-center gap-3 p-3 bg-navy-50 rounded-xl">
            ...
            <div>
              <p className="text-xs text-navy-400">{ L("Date de publication","تاريخ النشر") }</p>
              <p className="text-sm font-medium text-navy-700">
                {new Date(job.createdAt).toLocaleDateString(lang === "ar" ? "ar-MA" : lang === "en" ? "en-GB" : "fr-MA")}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3 p-3 bg-navy-50 rounded-xl">
            ...
            <div>
              <p className="text-xs text-navy-400">{ L("Candidatures","الطلبات") }</p>
        ...
        {job.sourceName && (
          <div className="mb-6 p-4 bg-navy-50 rounded-xl">
            <p className="text-xs text-navy-400 mb-1">{ L("Source de l'offre","المصدر") }</p>
        ...
        <div className="flex flex-wrap gap-3">
          {job.sourceUrl ? (
            ...
            >
              <Briefcase size={18} /> { L("Postuler sur","قدّم على المصدر") } {job.sourceName || "l'offre"}
