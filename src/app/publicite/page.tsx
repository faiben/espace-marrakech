    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div className="text-center mb-12">
        ...
        <p className="text-navy-600 max-w-2xl mx-auto">
          { L("Faites la promotion de votre activit\u00e9 sur notre plateforme et touchez des milliers de r\u00e9sidents \u00e0 Marrakech","اعلن عن نشاطك على منصتنا واصل إلى آلاف السكان في مراكش") }
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
        {positions.map((pos) => (
          <div key={pos.key} className="bg-white rounded-2xl border border-emerald-100 card-shadow p-6 text-center">
            ...
            <p className="text-sm text-navy-500 mb-2">{lang === "ar" ? pos.desc.ar : pos.desc.fr}</p>
            ...
            >
              {pkg.popular && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full">
                  { L("Le plus populaire","الأكثر طلبا") }
                </span>
              )}
              <div className={clsx("inline-flex items-center justify-center w-14 h-14 rounded-xl mb-4", pkg.color)}>
                {pkg.icon}
              </div>
              <h3 className="text-lg font-bold text-navy-800 mb-1">{lang === "ar" ? pkg.name.ar : pkg.name.fr}</h3>
              <p className="text-xl font-extrabold text-primary-600 mb-4">{lang === "ar" ? pkg.price.ar : pkg.price.fr}</p>
              <ul className="space-y-2 mb-6">
                {(lang === "ar" ? pkg.features.ar : pkg.features.fr).map((f, i) => (
              ...
              >
                { L("Demander un devis","طلب عرض أسعار") }
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-emerald-100 card-shadow p-6">
          {sent ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-navy-700 mb-1">{ L("Entreprise","الشركة") }</label>
              ...
              <div>
                <label className="block text-sm font-medium text-navy-700 mb-1">{ L("Emplacement","الموضع") }</label>
              ...
              <div>
                <label className="block text-sm font-medium text-navy-700 mb-1">{ L("Message","الرسالة") }</label>
